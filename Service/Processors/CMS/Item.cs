...
namespace net.vieapps.Services.Portals
{
	public static class ItemProcessor
	{
		...
		internal static async Task<JObject> CreateItemAsync(this RequestInfo requestInfo, bool isSystemAdministrator, CancellationToken cancellationToken)
		{
			...
			Utility.Cache.AddSetMemberAsync(item.ContentType.ObjectCacheKeys, item.GetCacheKey(), ServiceBase.ServiceComponent.CancellationToken).Run();
		...
		internal static async Task<JObject> DeleteItemAsync(this RequestInfo requestInfo, bool isSystemAdministrator, CancellationToken cancellationToken)
		{
			...
			// store object cache key to clear related cached
			Utility.Cache.RemoveSetMembersAsync(item.ContentType.ObjectCacheKeys, item.GetCacheKey(), ServiceBase.ServiceComponent.CancellationToken).Run();
		...
		internal static async Task<JObject> GenerateAsync(RequestInfo requestInfo, bool isSystemAdministrator, CancellationToken cancellationToken)
		{
			...
			else
			{
				...
				if (string.IsNullOrWhiteSpace(data))
				{
					...
					Task.WhenAll(
						Utility.Cache.SetAsync(cacheKey, data, ServiceBase.ServiceComponent.CancellationToken),
						@object.ContentType != null ? Utility.Cache.AddSetMemberAsync(@object.ContentType.ObjectCacheKeys, @object.GetCacheKey(), ServiceBase.ServiceComponent.CancellationToken) : Task.CompletedTask,
		...
		internal static async Task<JObject> SyncItemAsync(this RequestInfo requestInfo, CancellationToken cancellationToken)
		{
