...
namespace net.vieapps.Services.Portals
{
	public static class ItemProcessor
	{
		...
		internal static string GetCacheKeyOfAlias(this string contentTypeID, string alias)
		...
		internal static string GetCacheKeyOfAlias(this Item item)
			=> item?.ContentType?.ID?.GetCacheKeyOfAlias(item?.Alias);
		...
		static async Task<Item> RefreshAsync(this Item item, bool force, CancellationToken cancellationToken, bool reloadWebpages = false, bool writeLogs = false, string correlationID = null, string message = null)
		{
			if (force)
			{
				...
				await Utility.Cache.RemoveAsync(item.ReUpdate(), cancellationToken).ConfigureAwait(false);
				item = await Item.GetAsync(item.ID, cancellationToken).ConfigureAwait(false);
			}
			if (reloadWebpages)
				item.RebuildCacheAsync(true, correlationID, writeLogs, Utility.CancellationToken).Execute(ex => Utility.WriteErrorAsync(ex, $"Error occurred while rebuilding cache of '{item.Title}' [ID: {item.ID}] => {ex.Message}", "Caches", correlationID));
			return item;
		}

		internal static async Task ClearRelatedCacheAsync(this Item item, CancellationToken cancellationToken = default, string correlationID = null, bool clearDataCache = true, bool clearHtmlCache = false, bool doRefresh = true)
		{
			...
			if (item?.Organization != null && (item.Organization.ExamineURLs == null || item.Organization.ExamineURLs.Count < 1))
				item.RebuildCacheAsync(doRefresh, correlationID, writeLogs, Utility.CancellationToken).Execute(ex => Utility.WriteErrorAsync(ex, $"Error occurred while rebuilding cache of '{item.Title}' [ID: {item.ID}] => {ex.Message}", "Caches", correlationID));
		}
		...
		internal static async Task<(List<Item> Objects, long TotalRecords, JToken Thumbnails)> SearchAsync(this RequestInfo requestInfo, string query, IFilterBy<Item> filter, SortBy<Item> sort, int pageSize, int pageNumber, string contentTypeID = null, long totalRecords = -1, bool processCache = true, CancellationToken cancellationToken = default, bool searchThumbnails = true)
		{
			...
			totalRecords = totalRecords > -1
				...
				: string.IsNullOrWhiteSpace(query)
					? await Item.CountAsync(filter, contentTypeID, processCache, cacheKeyOfTotalObjects, cancellationToken).ConfigureAwait(false)
					: await Item.CountAsync(query, filter, contentTypeID, cancellationToken).ConfigureAwait(false);

			// search objects
			var objects = totalRecords > 0
				? string.IsNullOrWhiteSpace(query)
					? await Item.FindAsync(filter, sort, pageSize, pageNumber, contentTypeID, processCache, cacheKeyOfObjects, cancellationToken).ConfigureAwait(false)
			...
			Task.WhenAll
			(
				Utility.Cache.SetAsync(objects.ToDictionary(@object => @object.GetCacheKeyOfAlias(), @object => @object.ID), null, 0, Utility.CancellationToken),
				contentType != null
					? Task.WhenAll
					(
		...
		internal static async Task<JObject> SearchItemsAsync(this RequestInfo requestInfo, bool isSystemAdministrator, CancellationToken cancellationToken)
		{
			...
			var (objects, totalRecords, thumbnails) = await requestInfo.SearchAsync(query, filter, sort, pageSize, pageNumber, contentType?.ID, pagination.TotalRecords > -1 ? pagination.TotalRecords : -1, requestInfo.IsCacheAvailable(), cancellationToken).ConfigureAwait(false);
			...
			if (string.IsNullOrWhiteSpace(query))
				...
				(
					...
					Utility.IsCacheLogEnabled
						? Utility.WriteLogAsync(requestInfo, $"Update cache when search for CMS.Items => {cacheKeyOfObjectsJson}", "Caches")
						: Task.CompletedTask
				).Execute();
		...
		internal static async Task<JObject> CreateItemAsync(this RequestInfo requestInfo, bool isSystemAdministrator, CancellationToken cancellationToken)
		{
			...
			Utility.NotRecognizedAliases.Remove(contentType.ID.GetItemAliasKey(item.Alias));
			...
			if (writeLogs)
				await requestInfo.WriteLogAsync($"Create new successful [{item.Title} - ID: {item.ID}]", "Caches").ConfigureAwait(false);
			...
			(
				...
				item.ClearRelatedCacheAsync(Utility.CancellationToken, requestInfo.CorrelationID, writeLogs),
			...
			).Execute(ex => Utility.WriteErrorAsync(ex, $"Error occurred while working with cache/task when create new ['{item.Title}'] => {ex.Message}", "Caches", requestInfo.CorrelationID));
		...
		internal static async Task<JObject> GetItemAsync(this RequestInfo requestInfo, bool isSystemAdministrator, CancellationToken cancellationToken)
		{
			...
			var item = (identity.IsValidUUID()
				? await Item.GetAsync(identity, requestInfo.IsCacheAvailable(), cancellationToken).ConfigureAwait(false)
				: await Item.GetItemByAliasAsync(requestInfo.GetParameter("RepositoryEntityID") ?? requestInfo.GetParameter("x-content-type-id"), identity, cancellationToken).ConfigureAwait(false)) ?? throw new InformationNotFoundException();
			...
			if (isRefresh)
			{
				...
				item = await item.RefreshAsync(true, cancellationToken, true, true, requestInfo.CorrelationID).ConfigureAwait(false);

				var privileges = item.WorkingPrivileges;
				var parentPrivileges = item.ContentType?.WorkingPrivileges;
				var isAdministrator = requestInfo.Session.User.IsAdministrator(privileges, parentPrivileges, item.Organization, false);
				var isModerator = requestInfo.Session.User.IsModerator(privileges, parentPrivileges, item.Organization, false);
				var isEditor = requestInfo.Session.User.IsEditor(privileges, parentPrivileges, item.Organization, false);
				var isContributor = requestInfo.Session.User.IsContributor(privileges, parentPrivileges, item.Organization, false);
				var isViewer = requestInfo.Session.User.IsViewer(privileges, parentPrivileges, item.Organization, false);
				await requestInfo.WriteLogAsync($"Refresh an individual CMS.Item [{item.Title}]" + "\r\n\r\n"
					...
					+ $"- Item: {item.ToJson()}" + "\r\n\r\n"
					+ $"- Content-Type: {item.ContentType?.ToJson()}" + "\r\n\r\n"
					+ $"- Module: {item.Module?.ToJson(false, false)}" + "\r\n\r\n"
					+ $"- Organization: {item.Organization?.ToJson(false, false)}"
				, "Caches").ConfigureAwait(false);
			}

			// store object cache key to clear related cached
			Utility.Cache.AddSetMemberAsync(item.ContentType.ObjectCacheKeys, item.GetCacheKey(), Utility.CancellationToken).Execute();
		...
		{
			await Item.UpdateAsync(item, requestInfo.Session.User.ID, requestInfo.IsCacheAvailable(), cancellationToken).ConfigureAwait(false);
			Utility.NotRecognizedAliases.Remove(item.ContentType.ID.GetItemAliasKey(item.Alias));
		...
		{
			// prepare
			var item = await Item.GetAsync(requestInfo.GetObjectIdentity() ?? "", requestInfo.IsCacheAvailable(), cancellationToken).ConfigureAwait(false) ?? throw new InformationNotFoundException();
			if (item.Organization == null || item.Module == null || item.ContentType == null)
				throw new InformationInvalidException("The organization/module/item-type is invalid");
		...
		internal static async Task<JObject> DeleteAsync(this Item item, RequestInfo requestInfo, bool updateCache, bool sendUpdatingMessages, CancellationToken cancellationToken)
		{
			...
			await Item.DeleteAsync(item.ID, requestInfo.Session.User.ID, cancellationToken).ConfigureAwait(false);
		...
		internal static async Task<JObject> GenerateAsync(RequestInfo requestInfo, bool isSystemAdministrator, CancellationToken cancellationToken)
		{
			...
			if (isList)
			{
				...
				// get cache
				long totalRecords = 0;
				data = Utility.IsCacheAvailable() ? await Utility.Cache.GetAsync<string>(cacheKeyOfObjectsXml, cancellationToken).ConfigureAwait(false) : null;

				// process if has no cache
				if (string.IsNullOrWhiteSpace(data))
				{
					// search
					var results = await requestInfo.SearchAsync(null, filter, sort, pageSize, pageNumber, contentTypeID, -1, Utility.IsCacheAvailable(), cancellationToken).ConfigureAwait(false);
					...
					(
						...
						contentType != null
							? Utility.Cache.AddSetMembersAsync(contentType.GetSetCacheKey(), objects.Select(@object => @object.GetCacheKeyOfAlias()).Concat([cacheKeyOfObjectsXml]), Utility.CancellationToken)
							: Task.CompletedTask,
						Utility.IsCacheLogEnabled
							? Utility.WriteLogAsync(requestInfo, $"Update related keys into Content-Type's set when generate collection of CMS.Item [{contentType?.Title} - ID: {contentType?.ID} - Set: {contentType?.GetSetCacheKey()}]", "Caches")
							: Task.CompletedTask
					).Execute();
				}
				else if (showPagination)
				{
					totalRecords = Utility.IsCacheAvailable() ? await Utility.Cache.GetAsync<long>(cacheKeyOfTotalObjects, cancellationToken).ConfigureAwait(false) : -1;
					if (totalRecords < 1)
					{
						totalRecords = await Item.CountAsync(filter, contentTypeID, Utility.IsCacheAvailable(), cacheKeyOfTotalObjects, cancellationToken).ConfigureAwait(false);
						if (contentType != null)
			...
			else
			{
				...
				data = Utility.IsCacheAvailable() ? await Utility.Cache.GetAsync<string>(cacheKey, cancellationToken).ConfigureAwait(false) : null;
				...
				if (string.IsNullOrWhiteSpace(data))
				{
					...
					if (showOthers)
					{
						...
						).Prepare(requestInfo), null, numberOfOthers, 1, contentTypeID, Utility.IsCacheAvailable(), null, cancellationToken);
		...
		internal static async Task<JObject> SyncItemAsync(this RequestInfo requestInfo, CancellationToken cancellationToken, bool sendNotifications = false, bool dontCreateNewVersion = false)
		{
			...
			var item = await Item.GetAsync(data.Get<string>("ID"), requestInfo.IsCacheAvailable(), cancellationToken).ConfigureAwait(false);
			...
			if (!@event.IsEquals("Delete"))
			{
				...
				else
				{
					...
					if (existing != null && !existing.ID.IsEquals(item.ID))
						item.Alias = $"{item.Alias}-{DateTime.Now.ToUnixTimestamp()}-{UtilityService.GetRandomNumber()}";
					await Item.UpdateAsync(item, dontCreateNewVersion, requestInfo.IsCacheAvailable(), cancellationToken).ConfigureAwait(false);
				}
				Utility.NotRecognizedAliases.Remove(item.ContentType.ID.GetItemAliasKey(item.Alias));
				await Utility.Cache.SetAsync(item.GetCacheKeyOfAlias(), item.ID, cancellationToken).ConfigureAwait(false);
			}
			else if (item != null)
				await Item.DeleteAsync(item.ID, item.LastModifiedID, cancellationToken).ConfigureAwait(false);
				...
				item.ContentType != null
					...
					(
						@event.IsEquals("Delete")
							...
							: Utility.Cache.AddSetMemberAsync(item.ContentType.ObjectCacheKeys, item.GetCacheKey(), Utility.CancellationToken),
						sendNotifications
							? item.SendNotificationAsync(@event, item.ContentType.Notifications, oldStatus, item.Status, requestInfo, Utility.CancellationToken)
							: Task.CompletedTask
					)
					: Task.CompletedTask
			).Execute();
		...
		{
			// prepare
			var item = await Item.GetAsync(requestInfo.GetObjectIdentity() ?? "", requestInfo.IsCacheAvailable(), cancellationToken).ConfigureAwait(false);
			if (item == null)
				throw new InformationNotFoundException();
			...
			// rollback
			var oldStatus = item.Status;
			item = await Item.RollbackAsync(requestInfo.GetParameter("x-version-id") ?? "", requestInfo.Session.User.ID, cancellationToken).ConfigureAwait(false);
			Utility.NotRecognizedAliases.Remove(item.ContentType.ID.GetItemAliasKey(item.Alias));
			...
			(
				item.ClearRelatedCacheAsync(Utility.CancellationToken, requestInfo.CorrelationID),
				item.SendNotificationAsync("Rollback", item.ContentType.Notifications, oldStatus, item.Status, requestInfo, Utility.CancellationToken),
				Utility.Cache.SetAsync(item, Utility.CancellationToken),
				Utility.Cache.SetAsync(item.GetCacheKeyOfAlias(), item.ID, Utility.CancellationToken)
			).Execute();
		...
		internal static async Task<JToken> ProcessItemMcpRequestAsync(this RequestInfo requestInfo, ContentType contentType, bool isSystemAdministrator = false, CancellationToken cancellationToken = default)
		{
			...
			if (verb.IsEquals("SEARCH"))
			{
				...
				var result = await new RequestInfo().SearchAsync(query, filter, sort, pageSize, pageNumber, contentType.ID, totalRecords, requestInfo.IsCacheAvailable(), cancellationToken).ConfigureAwait(false);
			...
			{
				var item = await Item.GetAsync(objectIdentity, cancellationToken).ConfigureAwait(false);
				var privileges = item?.WorkingPrivileges;
