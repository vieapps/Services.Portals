...
namespace net.vieapps.Services.Portals
{
	public static class ContentTypeProcessor
	{
		...
		internal static async Task ClearRelatedCacheAsync(this ContentType contentType, CancellationToken cancellationToken, string correlationID = null, bool clearDataCache = true, bool clearHtmlCache = true, bool doRefresh = true)
		{
			...
			(
				...
				doRefresh && (contentType.Organization.ExamineURLs == null || contentType.Organization.ExamineURLs.Count < 1)
					? contentType.Organization.RefreshWebPagesAsync([contentType.Organization.URL], 1, correlationID, $"Refresh when clear related cache of a content-type [{contentType.Title} - ID: {contentType.ID}]", true, cancellationToken)
					: Task.CompletedTask
			).ConfigureAwait(false);
		}
		...
		internal static async Task<JObject> DeleteAsync(this ContentType contentType, RequestInfo requestInfo, bool deleteContents, bool updateCache, bool sendUpdatingMessages, CancellationToken cancellationToken)
		{
			...
			await ContentType.DeleteAsync(contentType.ID, requestInfo.Session.User.ID, cancellationToken).ConfigureAwait(false);
		...
		internal static async Task<JObject> SyncContentTypeAsync(this RequestInfo requestInfo, CancellationToken cancellationToken, bool sendNotifications = false, bool dontCreateNewVersion = false)
		{
			...
			if (!@event.IsEquals("Delete"))
			{
				...
				await ContentType.DeleteAsync(contentType.ID, contentType.LastModifiedID, cancellationToken).ConfigureAwait(false);
