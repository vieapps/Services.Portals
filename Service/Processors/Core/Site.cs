	...
	public static class SiteProcessor
	{
		...
		internal static async Task ClearRelatedCacheAsync(this Site site, CancellationToken cancellationToken, string correlationID = null, bool clearDataCache = true, bool clearHtmlCache = true, bool doRefresh = true)
		{
			...
			if (doRefresh && (site.Organization.ExamineURLs == null || site.Organization.ExamineURLs.Count < 1))
				await site.Organization.RefreshWebPagesAsync([site.Organization.URL, site.GetURL(), $"{site.GetURL()}/favicon.ico", $"{site.Organization.FakePortalsHttpURI ?? Utility.PortalsHttpURI}/_css/s_{site.ID}.css", $"{site.Organization.FakePortalsHttpURI ?? Utility.PortalsHttpURI}/_js/s_{site.ID}.js", $"{Utility.PortalsHttpURI}/_css/s_{site.ID}.css", $"{Utility.PortalsHttpURI}/_js/s_{site.ID}.js"], correlationID, $"Refresh when clear related cache of a site [{site.Title} - ID: {site.ID}]", true, false, cancellationToken).ConfigureAwait(false);
		}
