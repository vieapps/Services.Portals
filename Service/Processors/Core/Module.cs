...
namespace net.vieapps.Services.Portals
{
	public static class ModuleProcessor
	{
		...
		internal static Task ClearCacheAsync(this Module module, CancellationToken cancellationToken, string correlationID = null, bool clearObjectsCache = true, bool clearRelatedDataCache = true, bool clearRelatedHtmlCache = true, bool doRefresh = true)
			=> Task.WhenAll
			(
				module.ClearRelatedCacheAsync(clearObjectsCache, clearRelatedDataCache, clearRelatedHtmlCache, doRefresh, correlationID, cancellationToken),
				...
				Utility.IsCacheLogEnabled
					? Utility.WriteLogAsync(correlationID, $"Clear cache of a module [{module.Title} - ID: {module.ID}]", "Caches")
					: Task.CompletedTask
			);
		...
		internal static async Task<JObject> CreateModuleAsync(this RequestInfo requestInfo, bool isSystemAdministrator, CancellationToken cancellationToken)
		{
			...
			Task.WhenAll
			(
				module.ClearRelatedCacheAsync(true, true, true, true, requestInfo.CorrelationID, Utility.CancellationToken),
		...
		internal static async Task<JObject> DeleteModuleAsync(this RequestInfo requestInfo, bool isSystemAdministrator, CancellationToken cancellationToken)
		{
			...
			// delete
			return await module.DeleteAsync(requestInfo, true, true, cancellationToken).ConfigureAwait(false);
		}
		...
		internal static async Task<JObject> DeleteAsync(this Module module, RequestInfo requestInfo, bool updateCache, bool sendUpdatingMessages, CancellationToken cancellationToken)
		{
			...
			if (updateCache && module.Organization?._moduleIDs != null)
			{
				...
				{
					var objectName = module.Organization.GetObjectName();
					var ojson = module.Organization.ToJson();
					new UpdateMessage
					{
						Type = $"{requestInfo.ServiceName}#{objectName}#Update",
						Data = ojson,
			...
			var json = sendUpdatingMessages ? module.ToJson() : null;
			if (sendUpdatingMessages)
			{
				var objectName = module.GetObjectName();
				new UpdateMessage
		...
		internal static async Task<JObject> SyncModuleAsync(this RequestInfo requestInfo, CancellationToken cancellationToken, bool sendNotifications = false, bool dontCreateNewVersion = false)
		{
			...
			if (requestInfo.GetHeaderParameter("x-converter") == null || @event.IsEquals("Delete"))
				...
				await module.ClearRelatedCacheAsync(true, true, true, true, requestInfo.CorrelationID, cancellationToken).ConfigureAwait(false);
