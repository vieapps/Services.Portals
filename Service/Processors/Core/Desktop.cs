	...
	public static class DesktopProcessor
	{
		...
		internal static async Task ClearRelatedCacheAsync(this Desktop desktop, string oldParentID, CancellationToken cancellationToken, string correlationID = null, bool clearDataCache = true, bool clearHtmlCache = true, bool doRefresh = false)
		{
			...
			if (doRefresh && (desktop.Organization.ExamineURLs == null || desktop.Organization.ExamineURLs.Count < 1))
				await desktop.Organization.RefreshWebPagesAsync([$"{desktop.Organization.URL}/{desktop.Alias}", desktop.ID.Equals(desktop.Organization.HomeDesktop?.ID) ? desktop.Organization.URL : null, $"{desktop.Organization.FakePortalsHttpURI ?? Utility.PortalsHttpURI}/_css/d_{desktop.ID}.css", $"{desktop.Organization.FakePortalsHttpURI ?? Utility.PortalsHttpURI}/_js/d_{desktop.ID}.js", $"{Utility.PortalsHttpURI}/_css/d_{desktop.ID}.css", $"{Utility.PortalsHttpURI}/_js/d_{desktop.ID}.js"], true, correlationID, $"Refresh when clear related cache of a desktop [{desktop.Title} - ID: {desktop.ID}]", cancellationToken).ConfigureAwait(false);
		}
