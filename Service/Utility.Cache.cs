...
namespace net.vieapps.Services.Portals
{
	public static partial class Utility
	{
		...
		internal static void SendClearFilesCacheRequest(this IPortalObject @object, string correlationID)
		...
		/// </summary>
		public static void SendInvalidateL1CacheMessage(this string url, string cacheKey, string portalsHttpURI = null, bool writeLogs = false, string correlationID = null)
		{
			var data = new JObject
			{
				["Key"] = cacheKey,
				["URL"] = url,
				["PortalsHttpURI"] = portalsHttpURI ?? Utility.PortalsHttpURI
			};
			if (writeLogs)
				data["X-Logs"] = true;
		...
		public static async Task PurgeDesktopCacheByURLsAsync(this IEnumerable<Desktop> desktops, string correlationID, bool writeLogs, CancellationToken cancellationToken)
		{
			...
			await desktops.ForEachAsync(async (desktop, cancellationtoken) =>
			{
				...
				paths.ForEach(path => data.Add(($"{organization.ID}:{path.GenerateUUID()}", $"~{path}{suffix}")));
			}, cancellationToken, true, false).ConfigureAwait(false);

			var cacheKeys = data.Select(info => info.Key)
				.Distinct(StringComparer.OrdinalIgnoreCase)
				.Select(cacheKey => new[] { cacheKey, $"{cacheKey}:time", $"{cacheKey}:expiration" })
				.SelectMany(cacheKey => cacheKey)
				.Concat(desktops.Select(desktop =>
				{
					var cacheKey = desktop.GetDesktopCacheKey(organization.GetURL(false, desktop.Alias + suffix));
					return new[] { cacheKey, $"{cacheKey}:time", $"{cacheKey}:expiration" };
				}).SelectMany(cacheKey => cacheKey));
			...
			var siteURL = organization.GetSiteURL();
		...
		public static async Task PurgeCDNCacheAsync(
			...
			Dictionary<string, string> refreshHeaders,
		...
		)
		{
			...
			if (doRefresh)
			{
				...
				{
					refreshHeaders = new Dictionary<string, string>
					{
				...
				delayBeforeRefresh = gotCDN ? delayBeforeRefresh > 0 ? delayBeforeRefresh : (Utility.CDNDelaySeconds * 1234) + UtilityService.GetRandomNumber(13, 31) : 0;
				if (writeLogs)
					await Utility.WriteLogAsync(correlationID, $"Refresh when purge CDN cache [{organization.Title}]\r\nURLs: [{urls.Count():###,##0}]\r\n{urls.Join("\r\n")}\r\nSettings - Parallels: {Utility.RunProcessorInParallelsMode} - Wait: {waitForRefreshen} - Delay: {delayBeforeRefresh:###,###}ms - Headers: {refreshHeaders?.ToJson()}", "Caches").ConfigureAwait(false);

				if (waitForRefreshen)
					await organization.RefreshWebPagesAsync(urls, true, refreshHeaders, delayBeforeRefresh, true, correlationID, $"Refresh when purge CDN cache (wait) [{organization.Title}]", writeLogs, cancellationToken).ConfigureAwait(false);
				else
		...
		/// Purges cache of this organization at CDN by specified URLs
		/// </summary>
		public static Task PurgeCDNCacheAsync(
			this Organization organization,
			IEnumerable<string> urls,
			bool doRefresh,
			int delayBeforeRefresh,
		...
		{
			var siteURL = organization.GetSiteURL();
			return organization.PurgeCDNCacheAsync(urls?.Select(url => url?.Replace("~/", siteURL)), doRefresh, delayBeforeRefresh, waitForRefreshen, null, correlationID, writeLogs, cancellationToken, onCompleted);
		}
		...
		)
		{
			...
			if (organization != null)
				await organization.PurgeCDNCacheAsync(new[] { organization.GetSiteURL() }.Concat(urls ?? []), doRefresh && (organization.ExamineURLs == null || organization.ExamineURLs.Count < 1), 0, false, correlationID, writeLogs, cancellationToken).ConfigureAwait(false);
		}
		...
		public static Task PurgeCDNCacheAsync(
			...
			CancellationToken cancellationToken,
			Action<IEnumerable<string>> onCompleted = null
		) => @object.PurgeCDNCacheAsync([], doRefresh, correlationID, writeLogs, cancellationToken, onCompleted);
		...
		public static async Task RefreshWebPagesAsync(this Organization organization, IEnumerable<string> urls, bool doNormalize, Dictionary<string, string> headers, int delayBeforeStart, bool delayEachItemByIndex, string correlationID, string message, bool writeLogs, CancellationToken cancellationToken)
		{
			...
			var gotCDN = organization.GotCDN(true);
			var refreshURLs = (urls ?? []).Where(url => !string.IsNullOrWhiteSpace(url) && (url.StartsWith("~/") || url.Contains("://")));

			if (doNormalize)
				refreshURLs = refreshURLs.Select(url => url.GetPaginatingURLs(Utility.RefreshMaxPage, organization.AlwaysUseHtmlSuffix ? ".html" : "")).SelectMany(url => url)
					.Select(url => new[] { url, gotCDN && url.IsContains("//www.") ? url.Replace("//www.", "//") : null }).SelectMany(url => url);

			var siteURL = organization.GetSiteURL();
			refreshURLs = refreshURLs.Where(url => !string.IsNullOrWhiteSpace(url)).Select(url => url.Replace("~/", siteURL)).Distinct(StringComparer.OrdinalIgnoreCase).ToList();
		...
		public static Task RefreshWebPagesAsync(this Organization organization, IEnumerable<string> urls, bool force, string correlationID, string message, CancellationToken cancellationToken)
			=> organization.RefreshWebPagesAsync(urls, true, new Dictionary<string, string> { [force ? "x-no-cache" : "x-sliding-cache"] = "1" }, 0, false, correlationID, message, false, cancellationToken);
		...
		static async Task RebuildCacheAsync(this Organization organization, IEnumerable<string> urls, bool doRefresh, bool waitForRefreshen, string correlationID, string message, bool writeLogs, CancellationToken cancellationToken, Action<IEnumerable<string>> onRebuilt = null, Action<IEnumerable<string>> onRefreshen = null)
		{
			...
			// phase-1: refresh to rebuild cache
			...
			var gotCDN = organization.GotCDN(true);
			var siteURL = organization.GetSiteURL();
			workingURLs = (gotCDN ? urls.Where(url => url.IsEndsWith("/index.html")).Select(url => url.Left(url.Length - 10).Replace("~/", siteURL)) : []).Concat(urls.Select(url => url.Replace("~/", siteURL)));
			if (gotCDN && !Utility.CDNPurgeEverythingOnObject)
			{
				...
				await organization.PurgeCDNCacheAsync(workingURLs, doRefresh, delayMilliseconds, waitForRefreshen, refreshHeaders, correlationID, writeLogs, cancellationToken, onRefreshen).ConfigureAwait(false);
				if (writeLogs)
		...
		public static async Task RebuildCacheAsync(this Organization organization, IEnumerable<string> priorityURLs, IEnumerable<string> otherURLs, bool doRefresh, bool waitForRefreshen, string correlationID, string message, bool writeLogs, CancellationToken cancellationToken, Action<IEnumerable<string>> onRebuilt = null, Action<IEnumerable<string>> onRefreshen = null)
		{
			...
			if (gotCDN && doRefresh && Utility.CDNPurgeEverythingOnObject)
			{
				...
				await organization.RefreshWebPagesAsync(priorityURLs, true, null, (Utility.CDNDelaySeconds * 1234) + UtilityService.GetRandomNumber(13, 31), true, correlationID, message, writeLogs, cancellationToken).ConfigureAwait(false);
				if (writeLogs)
		...
		public static async Task RebuildCacheAsync(this IBusinessObject @object, bool doRefresh, string correlationID, bool writeLogs, CancellationToken cancellationToken, Action<IBusinessObject> onCompleted = null)
		{
			...
			if (@object.Organization is Organization organization)
			{
				...
				var defaultSite = organization.GetSite();
				var otherURLs = (organization.Sites ?? [])
					.Where(siteObj => siteObj != null && siteObj.ID != defaultSite?.ID && (siteObj.Status == ApprovalStatus.Approved || siteObj.Status == ApprovalStatus.Published))
					.Select(siteObj => organization.GetURL(false, siteObj, organization.AlwaysUseHtmlSuffix ? "/index.html" : "/"))
					.Distinct(StringComparer.OrdinalIgnoreCase)
					.ToList();
				...
				if (canPublish)
				{					
					var examinations = organization.ExamineURLs?.Where(exam => exam != null);
					if (examinations != null && examinations.Count() > 0)
					{
						var now = DateTime.Now;
						var path = @object.GetURL().Replace("~/", "/");
						...
						canPublish = examinations != null && examinations.Any();
					}
				}
				...
				else if (@object is Content content)
				{
					...
					await (content.Relateds ?? []).ForEachAsync(async id => relateds.Add(await Content.GetAsync(id, Utility.IsCacheAvailable(), cancellationToken).ConfigureAwait(false)), true, false).ConfigureAwait(false);
					otherURLs.AddRange(relateds.Where(contentObj => contentObj != null && contentObj.Status == ApprovalStatus.Published).Select(contentObj => contentObj.GetURL()));
		...
		internal static async Task<(IEnumerable<string> DataCacheKeys, IEnumerable<string> HtmlCacheKeys)> GetCacheKeysAsync(this Category category, bool getDataCacheKeys, bool getHtmlCacheKeys, CancellationToken cancellationToken)
		{
			...
			if (getDataCacheKeys)
			{
				...
				while (parentCategory != null)
				{
					cacheKeys = cacheKeys.Concat([Extensions.GetCacheKey(CategoryProcessor.GetCategoriesFilter(category.SystemID, category.RepositoryID, category.RepositoryEntityID, parentCategory.ID), sort, 0, 1)]);
					parentCategory = parentCategory.ParentCategory;
				}
		...
		internal static async Task<(IEnumerable<string> DataCacheKeys, IEnumerable<string> HtmlCacheKeys)> GetCacheKeysAsync(this IBusinessObject @object, bool getDataCacheKeys, bool getHtmlCacheKeys, CancellationToken cancellationToken)
		{
			...
			{
				var (_, _, contentTypeDataCacheKeys, contentTypeHtmlCacheKeys) = await contentType.GetCacheKeysAsync(true, true, true, cancellationToken).ConfigureAwait(false);
				if (getDataCacheKeys)
				{
					var cacheKeys = Extensions.GetRelatedCacheKeys(@object.GetCacheKey()).Concat(contentTypeDataCacheKeys);
					...
					if (@object is Content content)
					{
						...
						await new[] { content.CategoryID }.Concat(content.OtherCategories ?? []).ForEachAsync(async (categoryID, cancellationtoken) =>
						{
							...
							dataCacheKeys.AddRange(await Utility.Cache.GetSetMembersAsync(category.GetSetCacheKey(), cancellationtoken).ConfigureAwait(false) ?? []);
							if (category.ParentCategory == null)
							{
								...
								{
									dataCacheKeys.AddRange(await Utility.Cache.GetSetMembersAsync(desktop.GetSetCacheKey(), cancellationtoken).ConfigureAwait(false) ?? []);
					...
					else if (@object is Link link)
					{
						...
						while (parentLink != null)
						{
							cacheKeys = cacheKeys.Concat([Extensions.GetCacheKey(LinkProcessor.GetLinksFilter(link.SystemID, link.RepositoryID, link.RepositoryEntityID, parentLink.ID), sort, 0, 1)]);
							parentLink = parentLink.ParentLink;
						}
					}
						...
						await contentTypes.ForEachAsync(async (contentType, cancellationtoken) =>
						{
							...
							if (totalRecords > 0)
							{
								...
								while (pageNumber < totalPages && (maxContentPageNumber > 0 ? pageNumber < maxContentPageNumber : true))
								{
									...
									var relateds = new List<Content>();
									await contents.Select(content => content.Relateds ?? []).SelectMany(id => id).Distinct(StringComparer.OrdinalIgnoreCase).ForEachAsync(async id =>
									{
										relateds.Add(await Content.GetAsync(id, Utility.IsCacheAvailable(), cancellationtoken).ConfigureAwait(false));
									}, true, false).ConfigureAwait(false);
									var cacheKeys = contents.Select(content => content.GetCacheKeyOfAlias()).Concat([cacheKeyOfObjects]);
									await Task.WhenAll
									(
										Utility.Cache.AddSetMembersAsync(contentType.GetSetCacheKey(), cacheKeys.Concat(relateds.Where(content => content != null).Select(content => content.GetCacheKeyOfAlias())), cancellationtoken),
									...
									if (contents.Count > 0)
									{
										contentURLs.AddRange(contents.Where(content => minPublishedTime != null ? minPublishedTime.Value > content.PublishedTime.Value : true).Concat(relateds.Where(content => content != null && content.Status == ApprovalStatus.Published)).Select(content => content.GetURL()));
